...
export default function Home() {
  ...
  useGSAP(() => {
    if (view === 'search') {
      ...
      // 1. Hero Text & Input (Standard Fade Up)
      // We use .fromTo to ensure they don't get stuck at opacity 0
      ...
      if (heroInputRef.current) {
        gsap.fromTo(heroInputRef.current,
          { scale: 0.8, opacity: 0 },
      ...
      // We select all elements with class 'feature-card'
        ...
        gsap.fromTo(cards, 
          { y: 50, opacity: 0 }, // Start State
          {
            ...
            scrollTrigger: {
              ...
              start: "top 85%", // Triggers slightly earlier (safer)
  ...
  return (
    <main ref={containerRef} className="min-h-screen bg-[#FDF4FF] font-sans text-gray-900 pt-20 flex flex-col cursor-(--primary-gradient)">
      <Navbar />
      ...
      {view === 'search' ? (
        <div className="grow flex flex-col items-center justify-center min-h-[80vh] px-4 relative">
          ...
          {/* GSAP Target 1: Hero Text */}
          <div ref={heroTextRef} className="w-full max-w-3xl text-center space-y-8">
            ...
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter leading-[1.1]">
              ...
              <span className="text-transparent bg-clip-text bg-linear-to-r from-purple-600 to-pink-600">Dead Forks.</span>
            </h1>
            
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">
              Visualize repository history, identify active maintainers, and save your favorite diagrams. Free for open source.
            </p>

            {/* GSAP Target 2: Input Box */}
            <div 
              ref={heroInputRef}
        ...
        <div className="h-[calc(100vh-80px)] w-full relative bg-white border-t-2 border-black">
           ...
           {/* Added Embed Button from previous steps */}
           <button 
              onClick={() => {
                const repoName = lastRoot || "facebook/react"; 
                const activeCount = nodes.filter(n => n.data?.daysAgo < 30).length;
      ...
      {/* --- FEATURE SECTION --- */}
      {view === 'search' && (
        <section ref={featureSectionRef} id="features" className="py-20 px-6 border-t-2 border-black bg-white">
          <div className="max-w-6xl mx-auto">
            
            {/* Live Demo Section */}
            <div className="grid md:grid-cols-2 gap-12 items-center mb-24 feature-card">
               <div className="space-y-6">
                 <h2 className="text-4xl font-black tracking-tight">Don't just look. <br/> Interact with it.</h2>
                 <p className="text-lg text-gray-600">
                   Try it right now. This is a live visualization of <b>facebook/react</b> cached from our engine. Zoom in to see the contributors.
                 </p>
                 <ul className="space-y-3 font-bold">
                   <li className="flex items-center gap-2">
                     <div className="bg-green-100 p-1 border border-black rounded"><Zap size={16}/></div>
                     Live Zoom & Pan
                   </li>
                   <li className="flex items-center gap-2">
                     <div className="bg-purple-100 p-1 border border-black rounded"><Layout size={16}/></div>
                     Real GitHub Data
                   </li>
                 </ul>
               </div>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {[
                { title: 'Timeline View', icon: Clock, desc: 'See forks arranged by time. Who forked first?' },
                { title: 'Save & Share', icon: Star, desc: 'Logged in users can save up to 4 graphs.' },
                { title: 'Daily Quotas', icon: Zap, desc: 'Smart rate limiting ensures fair usage.' }
              ].map((feature, i) => (
                <div 
                  key={i} 
